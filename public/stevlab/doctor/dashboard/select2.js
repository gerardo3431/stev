$(function() {
    ...
    var paciente_general = $('#paciente_general').select2({
        placeholder: 'Buscar paciente',
        maximumSelectionLength: 1,
        minimumInputLength: 3,
        ajax:{
            url: '/stevlab/catalogo/getPacientesGeneral',
            type: 'get',
            delay: '200',
            data: function(params){
                return {
    ...
    paciente_general.on('select2:select', function(e){
        let paciente = $('#paciente_general').val()[0];
        let folio    = $('#folio_general').val();

        const search = axios.post('/stevlab/historial/search-folios-general', {
            _token: CSRF_TOKEN,
            paciente: paciente,
            folio: folio
        })
...
var BindDataTableGeneral = function(response){
    $('#dataTableFolioGeneral').DataTable().destroy();
